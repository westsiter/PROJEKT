...
<h2>Zarządzaj Polecanymi Produktami</h2>
<table>
    <tr>
        <th>Zdjecie</th>
        <th>Nazwa</th>
    ...
    <?php
    $result = $conn->query("SELECT id, nazwa, cena, zdjecie FROM polecane");
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            echo "<tr>
                    ...
                    <td>{$row['nazwa']}</td>
                    <td>{$row['cena']}</td>
                    <td style='border-right: 0px'><a href='edit_polecane.php?id={$row['id']}'>Edytuj</a></td>
